# R7 EU Procurement BOM

Status: procurement BOM for the corrected R7 manufacturing candidate.

Use this with the R7 board files, not the older R5 files.
...
During BOM audit I found that R5 was DRC-clean but did not include the required external LM2596 buck-regulator components. R6 corrected that. During visual review, the Teensy 4.1 footprint asymmetry in R6 was found to be one pad short on the upper long row. R7 corrects the Teensy footprint and keeps the R6 power/transceiver fixes. It includes:
...
R6 added complete power parts; R7 also corrects the Teensy 4.1 footprint side-row pinout. R7 uses the selected communication ICs to parts that are better aligned with Teensy 4.1 3.3 V logic:
...
## R7 Teensy footprint correction

The Teensy 4.1 long side rows are intentionally asymmetric. R7 uses 23 pads on the upper long row and 24 pads on the lower long row, matching the PJRC pinout pattern used by this design:

- Upper row: GND, VIN, 23..13, GND, 41..33
- Lower row: GND, 0..12, 3.3V, 24..32

This replaces the R6 generated footprint, which had only 22 pads on the upper row and misplaced the mid-row power/GND position.
